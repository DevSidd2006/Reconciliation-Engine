...
const AnomalyAlerts = () => {
  ...
  const getAnomalyIcon = (type) => {
    switch (type) {
      ...
      case 'MISMATCH_SPIKE': return '🚨';
      case 'SOURCE_DELAY': return '⏰';
  ...
  const getSystemHealthColor = (health) => {
    switch (health) {
      ...
      case 'ATTENTION_REQUIRED': return 'var(--warning-orange)';
  ...
  const getSystemHealthIcon = (health) => {
    switch (health) {
      case 'HEALTHY': return '✅';
      case 'ATTENTION_REQUIRED': return '⚠️';
      ...
      <div className="card" style={{ marginTop: '16px' }}>
        <div style={{ 
          ...
          <div style={{ 
            ...
            <div>🔴 HIGH: Immediate attention required</div>
            <div>🟡 MEDIUM: Monitor closely</div>
            <div>🟢 LOW: Informational</div>
          </div>
        </div>
      </div>
    </div>
